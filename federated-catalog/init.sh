...
echo "✔️ EDC Vault token created: $EDC_TOKEN"
...
EDC_PARTICIPANT_ID=$(vault kv get -format=json -address="$VAULT_ADDR" "$MOUNT_PATH/edc.participant.id" | jq -r '.data.data.content')
echo "✔️ EDC Participant ID: $EDC_PARTICIPANT_ID"

EDC_IAM_STS_CLIENT_ID=$(vault kv get -format=json -address="$VAULT_ADDR" "$MOUNT_PATH/edc.participant.did" | jq -r '.data.data.content')
echo "✔️ EDC STS client id acquired"

EDC_PARTICIPANT_PUBLIC_KEY_ALIAS=$(vault kv get -format=json -address="$VAULT_ADDR" "$MOUNT_PATH/edc.participant.public.key.alias" | jq -r '.data.data.content')
echo "✔️ EDC public key acquired"

EDC_PARTICIPANT_PRIVATE_KEY_ALIAS=$(vault kv get -format=json -address="$VAULT_ADDR" "$MOUNT_PATH/edc.participant.private.key.alias" | jq -r '.data.data.content')
...
java -Dedc.fs.config="$CONFIG_PATH" \
     ...
     -Dedc.iam.sts.oauth.client.id=$EDC_IAM_STS_CLIENT_ID \
     ...
     -Dedc.iam.issuer.id=$EDC_IAM_STS_CLIENT_ID \
     -jar build/libs/federated-catalog.jar
